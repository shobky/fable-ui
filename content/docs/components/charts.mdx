...
lib/fable-ui/evals/show-chart.md
```
...
`components/fable-ui/charts/tool-definition.ts` defines the Zod schema, the Vercel AI SDK tool, and the renderer contract that turns a tool part into props for `<Charts />`.
