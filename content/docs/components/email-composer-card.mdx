...
Register only the AI SDK tool in the route, then render the full definition in the chat UI:
...
```tsx
import type { ToolPartLike } from "@/lib/fable-ui/core"
import { FableToolPart } from "@/lib/fable-ui/core/tool-renderer"
import { showEmailComposer } from "@/lib/fable-ui/tools/show-email-composer-tool"

const registry = { show_email_composer: showEmailComposer }

export function EmailToolPart({ part }: { part: ToolPartLike }) {
  return <FableToolPart part={part} registry={registry} />
}
```

There is no email-delivery handler in `FableToolPart`. The registered tool renders a user-controlled handoff only. For host-owned draft persistence, render `EmailComposerCard` directly and use its optional `onDraftChange` prop.
