...
The quickstart installs a working example at `/fable-chat` and `/api/fable-chat`. It uses mock mode by default so a clean app can render Fable tool parts without provider keys.
