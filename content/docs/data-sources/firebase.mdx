...
The same `auth` object passed to `createFirebaseDriver({ db, auth })` must be the app's live Firebase Auth client. Fable does not read Firebase Auth from `FableDataProvider` context and does not use `ctx.auth.userId` as a Firebase fallback.
...
export function FirebaseSessionSync({ session }: { session: any }) {
