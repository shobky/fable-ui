...
function readStoredSelection(): Partial<ProviderSelection> {
  ...
  try {
    return JSON.parse(window.localStorage.getItem(selectionStorageKey) || "{}") as Partial<ProviderSelection>;
  } catch {
    return {};
  }
}
...
function isSource(value: unknown): value is ProviderCredentialSource {
  return value === "server-env" || value === "browser-key" || value === "session-key" || value === "none";
}

function eligibleKeys(localKeys: StoredProviderKey[], provider: ProviderId, model: string) {
  ...
  return serverReadiness.map((provider) => {
    ...
    const modelReadiness = Object.fromEntries(
      models.map((model) => {
        ...
        const serverConfigured = provider.serverConfigured || provider.isConfigured;
        const source = serverConfigured ? "server-env" : matchingKeys.length > 0 ? "browser-key" : "none";
        const readiness: ModelReadiness = {
          ...
          selectedKeyId:
            provider.id === selectedProvider && model === selectedModel ? selectedKeyId : undefined,
        };
    ...
    return {
      ...
      isConfigured: provider.serverConfigured || provider.isConfigured || browserConfigured,
      modelReadiness,
    } satisfies ProviderReadiness;
  });
}

export function useProviderSettings({ defaults, serverReadiness }: UseProviderSettingsInput) {
  const storedSelection = useMemo(readStoredSelection, []);
  const [provider, setProviderState] = useState<ProviderId>(
    isProviderId(storedSelection.provider) ? storedSelection.provider : defaults.provider,
  );
  const [model, setModelState] = useState(storedSelection.model || defaults.model);
  const [credentialSource, setCredentialSourceState] = useState<ProviderCredentialSource>(
    isSource(storedSelection.credentialSource) ? storedSelection.credentialSource : "server-env",
  );
  const [selectedKeyId, setSelectedKeyIdState] = useState<string | undefined>(storedSelection.selectedKeyId);
  ...
  const refreshKeys = useCallback(async () => {
    try {
      ...
      setStoreError(error instanceof Error ? error.message : "Could not open encrypted browser key storage.");
    }
  }, []);
  ...
  const selectedProviderReadiness = readiness.find((candidate) => candidate.id === provider);
  const selectedModelReadiness = selectedProviderReadiness?.modelReadiness?.[model];
  const currentEligibleKeys = useMemo(() => eligibleKeys(localKeys, provider, model), [localKeys, model, provider]);

  useEffect(() => {
    const hasServerKey = Boolean(selectedProviderReadiness?.serverConfigured);
    const selectedKeyIsEligible = currentEligibleKeys.some((key) => key.id === selectedKeyId);
  ...
  }, [credentialSource, currentEligibleKeys, selectedKeyId, selectedProviderReadiness?.serverConfigured]);
  ...
  const setCredentialSource = useCallback((source: ProviderCredentialSource) => {
  ...
  const resolveRequestCredentials = useCallback(async () => {
    if (credentialSource !== "browser-key" && credentialSource !== "session-key") {
      return { credentialSource };
    }

    if (!selectedKeyId) {
      throw new Error("Select a saved browser key before sending a live provider request.");
    }
